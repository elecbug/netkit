...
func TestSlice(t *testing.T) {
	...
	for i := 0; i < iter; i++ {
		...
		t.Logf("general sort: %v, time: %fms", isScended(copy1), end)
		result[0] += end

		start = time.Now().UnixNano()
		slice.Sort(copy2, func(i, j int) bool { return i < j })
		end = float64(time.Now().UnixNano()-start) / 10e6
		t.Logf("go_type sort: %v, time: %fms", isScended(copy2), end)
		result[1] += end

		start = time.Now().UnixNano()
		slice.ParallelSort(copy3, func(i, j int) bool { return i < j }, 7)
		end = float64(time.Now().UnixNano()-start) / 10e6
		t.Logf("go_type parallel sort: %v, time: %fms", isScended(copy3), end)
		result[2] += end
	}
